...
namespace NHSE.WinForms.Zebra.SegmentLayouts
{
    class DisplaySegmentLayout : ISegmentLayout
    {
        ...
        public DisplaySegmentLayout(Item rawItem, DisplaySegmentLayoutOptions options)
        {
            ...
            {
                ushort modelId = ItemConvertor.Instance.GetCreatureModel(itemInfo.ItemId);
                ...
                if (options.CreatureDropStyle.HasFlag(CreatureDropStyle.Model))
                    droppedItems.Add(ItemConvertor.Instance.ApplyPresentation(new Item(modelId), PresentationType.Dropped));
            }
            else
            {
                displayItemId = itemInfo.ItemId;
